...
function OwnerOrderCard({data}) {
  ...
  const handleUpdateStatus=async(orderId, shopId, status)=>{
    try {
      const result=await axios.post(`${userServiceUrl}/api/v1/orders/update-status/${orderId}/${shopId}`,{status},{withCredentials:true});
      console.log("All OrderInfo",result.data);
    ...
    <div className='bg-white rounded-lg shadow p-4 space-y-4'>
      <div>
        ...
        <p className='flex items-center gap-2 text-sm text-gray-600 mt-1'>
          <FaPhone/> <span>{data.user.mobile}</span></p>
      ...
      <div className='flex justify-between items-center mt-auto pt-3 border-t border-gray-100'>
        ...
        <select value={data.shopOrders.status} onChange={(e)=>handleUpdateStatus(data._id,data.shopOrders.shop._id,e.target.value)} className='rounded-md border px-3 py-1 text-sm focus:outline-none focus:ring-2 border-[#ff4d2d] text-[#ff4d2d]'>
      ...
      {data.shopOrders.status=="outfordelivery" && 
        <div className='mt-3 p-2 border rounded-lg text-sm bg-orange-50'>
          ...
          ):data.shopOrders.assignedDeliveryBoy?<div>{data.shopOrders.assignedDeliveryBoy.fullname}-{data.shopOrders.assignedDeliveryBoy.mobile}</div>:<div>Waiting Available Delivery</div>}
          </div>
      }
